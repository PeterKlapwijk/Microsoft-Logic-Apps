...
az deployment group create `
    ...
    --template-file ../"Receive Intune Service Messages in Teams"/bicep/IntuneServiceMsg-root.bicep `
    --verbose
...
$ManagedIdentity = az identity show --name IntuneServiceMsg-ManagedIdentity --resource-group $ResourceGroupName | ConvertFrom-Json
...
#Get appRoleIds
$ServiceHealthReadAll = az ad sp show --id $graphId --query "appRoles[?value=='ServiceHealth.Read.All'].id | [0]" -o tsv

$appRoleIds = $ServiceHealthReadAll
