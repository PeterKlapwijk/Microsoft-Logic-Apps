param workflows_WINDriverApprovalReport_name string
param connections_excelonlinebusiness_name string = 'excelonlinebusiness'
param connections_office365_name string = 'office365'
param connections_SharePointOnline_name string = 'SharePointOnline'
param resourceLocation string 
param userAssignedIdentities_WINDriverApprovalReport_Identity_name string = 'WINDriverApprovalReport-ManagedIdentity'

resource workflows_WINDriverApprovalReport_name_resource 'Microsoft.Logic/workflows@2019-05-01' = {
  name: workflows_WINDriverApprovalReport_name
  location: resourceLocation
  identity: {
    type: 'UserAssigned'
    userAssignedIdentities: {
      '${resourceId('Microsoft.ManagedIdentity/userAssignedIdentities/',userAssignedIdentities_WINDriverApprovalReport_Identity_name)}': {}
    }
  }
  properties: {
    state: 'Enabled'
    definition: {
      '$schema': 'https://schema.management.azure.com/providers/Microsoft.Logic/schemas/2016-06-01/workflowdefinition.json#'
      contentVersion: '1.0.0.0'
      parameters: {
        '$connections': {
          defaultValue: {
          }
          type: 'Object'
        }
      }
      triggers: {
        Recurrence: {
          recurrence: {
            frequency: 'Month'
            interval: 1
          }
          evaluatedRecurrence: {
            frequency: 'Month'
            interval: 1
          }
          type: 'Recurrence'
        }
      }
      actions: {
        Compose_Excel_file: {
          runAfter: {
          }
          type: 'Compose'
          inputs: {
            '$content': ''
            '$content-type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
          }
        }
        Create_file: {
          runAfter: {
            Compose_Excel_file: [
              'Succeeded'
            ]
          }
          type: 'ApiConnection'
          inputs: {
            body: '@outputs(\'Compose_Excel_file\')'
            host: {
              connection: {
                name: '@parameters(\'$connections\')[\'sharepointonline\'][\'connectionId\']'
              }
            }
            method: 'post'
            path: '/datasets/@{encodeURIComponent(encodeURIComponent(\'\'))}/files'
            queries: {
              folderPath: '/'
              name: 'WindowsDriverApprovalReport-@{formatDateTime(utcNow(), \'yyyy-MM-dd-HH-mm\')}.xlsx'
              queryParametersSingleEncoded: true
            }
          }
          runtimeConfiguration: {
            contentTransfer: {
              transferMode: 'Chunked'
            }
          }
        }
        Delay: {
          runAfter: {
            For_each_recommended_windowsDriverUpdateProfile: [
              'Succeeded'
            ]
          }
          type: 'Wait'
          inputs: {
            interval: {
              count: 2
              unit: 'Minute'
            }
          }
        }
        For_each_recommended_windowsDriverUpdateProfile: {
          foreach: '@body(\'Parse_JSON_GET_windowsDriverUpdateProfiles\')?[\'value\']'
          actions: {
            Create_table: {
              runAfter: {
                Create_worksheet: [
                  'Succeeded'
                ]
              }
              type: 'ApiConnection'
              inputs: {
                body: {
                  ColumnsNames: 'name,version,manufacturer,releaseDateTime,applicableDeviceCount,approvalStatus,ProfiledisplayName'
                  Range: '\'@{body(\'Create_worksheet\')?[\'name\']}\'!A1:G1'
                  TableName: 'WindowsDrivers@{rand(1,100)}'
                }
                host: {
                  connection: {
                    name: '@parameters(\'$connections\')[\'excelonlinebusiness\'][\'connectionId\']'
                  }
                }
                method: 'post'
                path: '/drives/@{encodeURIComponent(\'GiEf4m1Lr4uhPnNqEYLk9Umq9Zot084gERI2gxgrytSuA\')}/files/@{encodeURIComponent(encodeURIComponent(body(\'Create_file\')?[\'Id\']))}/tables'
                queries: {
                  source: '/'
                }
              }
            }
            Create_worksheet: {
              runAfter: {
                Parse_JSON_GET_recommended_driverInventories: [
                  'Succeeded'
                ]
              }
              type: 'ApiConnection'
              inputs: {
                body: {
                  name: 'Sheet@{rand(1,100)}'
                }
                host: {
                  connection: {
                    name: '@parameters(\'$connections\')[\'excelonlinebusiness\'][\'connectionId\']'
                  }
                }
                method: 'post'
                path: '/codeless/v1.0/drives/@{encodeURIComponent(\'3S5PdGiEf4m1Lr4uhPnNqEYLk9Umq9Zot084gERI2gxgrytSuA\')}/items/@{encodeURIComponent(encodeURIComponent(body(\'Create_file\')?[\'Id\']))}/workbook/worksheets'
                queries: {
                  source: '/'
                }
              }
            }
            For_each_approvalStatus: {
              foreach: '@body(\'Parse_JSON_GET_recommended_driverInventories\')?[\'value\']'
              actions: {
                Condition: {
                  actions: {
                    Add_a_row_into_a_table: {
                      runAfter: {
                        Compose_Row: [
                          'Succeeded'
                        ]
                      }
                      type: 'ApiConnection'
                      inputs: {
                        body: '@outputs(\'Compose_Row\')'
                        host: {
                          connection: {
                            name: '@parameters(\'$connections\')[\'excelonlinebusiness\'][\'connectionId\']'
                          }
                        }
                        method: 'post'
                        path: '/codeless/v1.2/drives/@{encodeURIComponent(\'b!03bMWm1Z50up7V3S5PdGiEf4m1Lr4uhPnNqEYLk9Umq9Zot084gERI2gxgrytSuA\')}/items/@{encodeURIComponent(encodeURIComponent(body(\'Create_file\')?[\'Id\']))}/workbook/tables/@{encodeURIComponent(body(\'Create_table\')?[\'name\'])}/rows'
                        queries: {
                          source: '/'
                        }
                      }
                    }
                    Compose_Row: {
                      runAfter: {
                      }
                      type: 'Compose'
                      inputs: {
                        ProfiledisplayName: '@items(\'For_each_recommended_windowsDriverUpdateProfile\')?[\'displayName\']'
                        applicableDeviceCount: '@items(\'For_each_approvalStatus\')?[\'applicableDeviceCount\']'
                        approvalStatus: '@items(\'For_each_approvalStatus\')?[\'approvalStatus\']'
                        manufacturer: '@items(\'For_each_approvalStatus\')?[\'manufacturer\']'
                        name: '@items(\'For_each_approvalStatus\')?[\'name\']'
                        releaseDateTime: '@items(\'For_each_approvalStatus\')?[\'releaseDateTime\']'
                        version: '@items(\'For_each_approvalStatus\')?[\'version\']'
                      }
                    }
                  }
                  runAfter: {
                  }
                  expression: {
                    and: [
                      {
                        equals: [
                          '@items(\'For_each_approvalStatus\')?[\'approvalStatus\']'
                          'needsReview'
                        ]
                      }
                    ]
                  }
                  type: 'If'
                }
              }
              runAfter: {
                Create_table: [
                  'Succeeded'
                ]
              }
              type: 'Foreach'
            }
            HTTP_GET_recommended_driverInventories: {
              runAfter: {
              }
              type: 'Http'
              inputs: {
                authentication: {
                  audience: 'https://graph.microsoft.com'
                  identity: resourceId('Microsoft.ManagedIdentity/userAssignedIdentities', userAssignedIdentities_WINDriverApprovalReport_Identity_name)
                  type: 'ManagedServiceIdentity'
                }
                method: 'GET'
                uri: 'https://graph.microsoft.com/beta/deviceManagement/windowsDriverUpdateProfiles/@{items(\'For_each_recommended_windowsDriverUpdateProfile\')?[\'id\']}/driverInventories?$filter=category%20eq%20%27recommended%27'
              }
            }
            Parse_JSON_GET_recommended_driverInventories: {
              runAfter: {
                HTTP_GET_recommended_driverInventories: [
                  'Succeeded'
                ]
              }
              type: 'ParseJson'
              inputs: {
                content: '@body(\'HTTP_GET_recommended_driverInventories\')'
                schema: {
                  properties: {
                    '@@odata.context': {
                      type: 'string'
                    }
                    '@@odata.count': {
                      type: 'integer'
                    }
                    value: {
                      items: {
                        properties: {
                          applicableDeviceCount: {
                            type: 'integer'
                          }
                          approvalStatus: {
                            type: 'string'
                          }
                          category: {
                            type: 'string'
                          }
                          deployDateTime: {
                            type: 'string'
                          }
                          driverClass: {
                            type: 'string'
                          }
                          id: {
                            type: 'string'
                          }
                          manufacturer: {
                            type: 'string'
                          }
                          name: {
                            type: 'string'
                          }
                          releaseDateTime: {
                            type: 'string'
                          }
                          version: {
                            type: 'string'
                          }
                        }
                        required: [
                          'id'
                          'name'
                          'version'
                          'manufacturer'
                          'releaseDateTime'
                          'driverClass'
                          'applicableDeviceCount'
                          'approvalStatus'
                          'category'
                          'deployDateTime'
                        ]
                        type: 'object'
                      }
                      type: 'array'
                    }
                  }
                  type: 'object'
                }
              }
            }
          }
          runAfter: {
            Parse_JSON_GET_windowsDriverUpdateProfiles: [
              'Succeeded'
            ]
          }
          type: 'Foreach'
        }
        Get_file_content: {
          runAfter: {
            Delay: [
              'Succeeded'
            ]
          }
          type: 'ApiConnection'
          inputs: {
            host: {
              connection: {
                name: '@parameters(\'$connections\')[\'sharepointonline\'][\'connectionId\']'
              }
            }
            method: 'get'
            path: '/datasets/@{encodeURIComponent(encodeURIComponent(\'\'))}/files/@{encodeURIComponent(body(\'Create_file\')?[\'Id\'])}/content'
            queries: {
              inferContentType: true
            }
          }
        }
        HTTP_GET_windowsDriverUpdateProfiles: {
          runAfter: {
            Create_file: [
              'Succeeded'
            ]
          }
          type: 'Http'
          inputs: {
            authentication: {
              audience: 'https://graph.microsoft.com'
              identity: resourceId('Microsoft.ManagedIdentity/userAssignedIdentities', userAssignedIdentities_WINDriverApprovalReport_Identity_name)
              type: 'ManagedServiceIdentity'
            }
            method: 'GET'
            uri: 'https://graph.microsoft.com/beta/deviceManagement/windowsDriverUpdateProfiles'
          }
        }
        Parse_JSON_GET_windowsDriverUpdateProfiles: {
          runAfter: {
            HTTP_GET_windowsDriverUpdateProfiles: [
              'Succeeded'
            ]
          }
          type: 'ParseJson'
          inputs: {
            content: '@body(\'HTTP_GET_windowsDriverUpdateProfiles\')'
            schema: {
              properties: {
                '@@odata.context': {
                  type: 'string'
                }
                value: {
                  items: {
                    properties: {
                      approvalType: {
                        type: 'string'
                      }
                      createdDateTime: {
                        type: 'string'
                      }
                      deploymentDeferralInDays: {
                      }
                      description: {
                        type: 'string'
                      }
                      deviceReporting: {
                        type: 'integer'
                      }
                      displayName: {
                        type: 'string'
                      }
                      id: {
                        type: 'string'
                      }
                      inventorySyncStatus: {
                      }
                      lastModifiedDateTime: {
                        type: 'string'
                      }
                      newUpdates: {
                        type: 'integer'
                      }
                      roleScopeTagIds: {
                        items: {
                          type: 'string'
                        }
                        type: 'array'
                      }
                    }
                    required: [
                      'id'
                      'displayName'
                      'description'
                      'approvalType'
                      'deviceReporting'
                      'newUpdates'
                      'deploymentDeferralInDays'
                      'createdDateTime'
                      'lastModifiedDateTime'
                      'roleScopeTagIds'
                      'inventorySyncStatus'
                    ]
                    type: 'object'
                  }
                  type: 'array'
                }
              }
              type: 'object'
            }
          }
        }
        'Send_an_email_from_a_shared_mailbox_(V2)': {
          runAfter: {
            Get_file_content: [
              'Succeeded'
            ]
          }
          type: 'ApiConnection'
          inputs: {
            body: {
              Attachments: [
                {
                  ContentBytes: '@{base64(body(\'Get_file_content\'))}'
                  Name: '@body(\'Create_file\')?[\'Name\']'
                }
              ]
              Body: '<p>Hello IT Super Hero,<br>\n<br>\nAttached you\'ll find an overview of the Windows drivers and firmware that need to be reviewed.<br>\n<br>\nKind regards,<br>\n<br>\nIn The Cloud 24-7<br>\nhttps://inthecloud247.com</p>'
              Importance: 'Normal'
              MailboxAddress: ''
              Subject: 'Windows Drivers approval report'
              To: ''
            }
            host: {
              connection: {
                name: '@parameters(\'$connections\')[\'office365\'][\'connectionId\']'
              }
            }
            method: 'post'
            path: '/v2/SharedMailbox/Mail'
          }
        }
      }
      outputs: {
      }
    }
    parameters: {
      '$connections': {
        value: {
          excelonlinebusiness: {
            connectionId: resourceId('Microsoft.Web/connections', connections_excelonlinebusiness_name)
            connectionName: 'excelonlinebusiness'
            id: '${subscription().id}/providers/Microsoft.Web/locations/${resourceLocation}/managedApis/excelonlinebusiness'
          }
          office365: {
            connectionId: resourceId('Microsoft.Web/connections', connections_office365_name)
            connectionName: 'office365'
            id: '${subscription().id}/providers/Microsoft.Web/locations/${resourceLocation}/managedApis/office365'
          }
          sharepointonline: {
            connectionId: resourceId('Microsoft.Web/connections', connections_SharePointOnline_name)
            connectionName: 'SharePointOnline'
            id: '${subscription().id}/providers/Microsoft.Web/locations/${resourceLocation}/managedApis/SharePointOnline'
          }
        }
      }
    }
  }
}
